class DynamicLazyChoices:
    ...
    def _register(self, *choice):
        ...
        try:
            invalid = not all(cls._is_value(value) and cls._is_value(label) for value, label in group_choices)
            if invalid:
        ...
        if cls._is_value(self._choices, accept_promise=False):
            # TODO: volta o estado do choices ao que era antes de adicionar a nova choice
            # isso está sendo necessário pq o system check (impede do sistema iniciar se algum check falha)
    ...
    def _add_group_choice(self, group_choice):
        for i, existing_choice in enumerate(self._choices):
            if not self.is_group_choice(existing_choice):
                continue
            if existing_choice[self.GROUP_INDEX] == group_choice[self.GROUP_INDEX]:
                existing_choice[self.GROUP_CHOICES_INDEX] += group_choice[self.GROUP_CHOICES_INDEX]
                break
        else:
...
class ModelLazyChoices(DynamicLazyChoices):
    def __init__(self, value_attr='_choice_value', label_attr='_choice_label', group_attr='_choice_group'):
        super().__init__()
        self._models = {}
        self._displays_to_models = {}
        self.value_attr, self.label_attr, self.group_attr = value_attr, label_attr, group_attr
